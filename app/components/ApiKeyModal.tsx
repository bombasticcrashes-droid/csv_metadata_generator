...
export function ApiKeyModal({ open, onOpenChange }: ApiKeyModalProps) {
  ...
  const handleSave = () => {
    ...
    setApiKey(trimmed); // Save all keys as a single string
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter your Google Gemini API keys. You can add multiple keys (one per line) to increase your daily limit.
            If one key expires, the system will automatically use the next one.
          </DialogDescription>
        </DialogHeader>
